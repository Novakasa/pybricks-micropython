...
mp_obj_t pb_builtin_import(size_t n_args, const mp_obj_t *args) {
    ...
    void *modref;
    int frozen_type;
    char *module_path = m_new(char, qstr_len(module_name_qstr) + 4);
    strncpy(module_path, mp_obj_str_get_str(args[0]), qstr_len(module_name_qstr));
    strncpy(module_path + qstr_len(module_name_qstr), ".py", 4);
